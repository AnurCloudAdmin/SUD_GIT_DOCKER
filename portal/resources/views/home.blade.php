      ...
      <div class="row">
        <div class="col-12">
          <div class="card m-b-30">
            ...
            <form method="post" id="reportsdownload" name="reportsdownload" action="{{env('APP_URL')}}linksreportsdownload" style="display: inline-block;" > 
            @csrf
                      ...
                      <tr>
                        ...
                        <th>
                          ...
                          if($list->complete_status==1){
                            if($list->personal_disagree==0 && $list->policy_disagree==0){
                              echo "Completed";
                            }else{
                          ...
                          $medical_checked_response = (isset($list->medical_checked_response) && $list->medical_checked_response!= null) ? json_decode($list->medical_checked_response,true) : array();
                        ...
                        <td>
                          @if($list->complete_status==1)
							...
							@if($table=='links')
								<a href="{{url('/api/genPdf')}}/{{$list->proposal_no}}" target="_blank" download class="btn btn-primary">PDF</a>
							@else	
                        ...
                        <td>{{isset($params['email']) ? $params['email'] : ''}}</td>
                        <td>{{isset($params['Mobile_number']) ? $params['Mobile_number'] : ''}}</td>
                        <td>
                            @php
                              $languages = [
                                  'eng' => 'English',
                            ...
                            @if($list->sys_lang != null)
                                {{ $languages[$list->sys_lang] ?? 'Unknown Language' }}
                            @else
                              English
                            @endif
                        </td>
                        <td>{{isset($params['plan_details']['Plan_Name']) ? $params['plan_details']['Plan_Name'] : ''}}</td> 
                        <td>{{$list->created_at}}</td>
                        <th>{{($list->is_open==1)?'Open':'Not Open'}}</th>
                        <th>{{$list->is_open_at}}</th>
                        <th>{{($list->status==1 && $list->completed_status == 1)?'Expired':'Active'}}</th>
                       
                        <th>{{$list->personal_disagree}}</th>
                        <th>{{$list->policy_disagree}}</th>
                       <!-- <th></th>-->
                        <th>
                        @if($list->complete_status == 1)
