...
ApiController::logs($AppNumber, 'Short Link Api Success', $req, $response, "Success");
  ...
  public function base64_to_jpeg($base64_string, $output_file)
{
    ...
    if ($imageData === false || strlen($imageData) < 1000) {
        ...
        //ApiController::logs($proposal, 'Failed to decode or image too small', json_encode($post), '', "Success");
    ...
    try {
     ...
     // dd(strlen($url));
      ...
      elseif ($links['status'] == 0) {
        ...
        ApiController::logs($proposal,'validate PIVC Link Fail',json_encode(($post)),$result, "Fail");
...
if (File::isDirectory($versionFolder)) {
  ...
  public function addVideo(Request $request)
  {
   // dd($request->all());
    ...
    ApiController::logs($proposal, 'video capture started',"","",  "Success");
...
//dd(count($appendSuccess),$totalChunks);
  ...
  public function getFaceScore($proposal, $consentImage, $faceImage, $type = 'default')
  {
    ...
    ApiController::logs($proposal, 'get Face score start',"","",  "Success");
    ...
    ApiController::logs($proposal, 'get faces core end', json_encode($data), $result,  "Success");
 ...
 $responseArray['spchstatus'] = 'success'; //dd($responseArray);
  ...
  //  die;
      ...
      //$array = explode(",", $string);
       ...
       //dd($data);
        ApiController::logs($post['Application_Number'], 'create PIVC Link Started', json_encode($post), '', "Success");
